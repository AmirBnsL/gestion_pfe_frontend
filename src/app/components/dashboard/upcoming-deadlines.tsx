...
function UpcomingDeadlines({ deadlines, isLoaded }: UpcomingDeadlinesProps) {
  const deadlineItems = useMemo(() => {
    return deadlines.map((project, i) => (
      <motion.div
        key={i}
      ...
      >
        ...
        <div
          className={`absolute top-0 left-0 w-1 h-full ${
            project.status === "NEW" ? "bg-red-500" : "bg-green-500"
          }`}
        ></div>
  ...
  return (
    <div className="bg-slate-900/60 backdrop-blur-md rounded-xl border border-slate-800 p-6 shadow-[0_8px_30px_rgba(0,0,0,0.3)] relative overflow-hidden group">
      ...
      <div className="space-y-4">{deadlineItems}</div>
    </div>
  )
}

export default React.memo(UpcomingDeadlines)

...
import { sendInviteToSupervisor } from "./proposalsActions"
...
export function RequestsModal({
                                ...
                                isOpen,
                                onClose,
                                availableSupervisorsPromise = [],
                              }: {
  proposal: Project,
  isOpen: boolean,
  onClose: () => void,
  availableSupervisorsPromise: Promise<Teacher[]>}) {
  const availableSupervisors = use(availableSupervisorsPromise)
  ...
  const handleSupervisorAction = (requestId: number, action: "accept" | "reject") => {
  ...
  const handleStudentAction = (requestId: number, action: "accept" | "reject") => {
  ...
  const handleSendInvite = async (supervisorId: number) => {
    setInvitingIds((ids) => [...ids, supervisorId])
    try {
      await sendInviteToSupervisor(proposal.id, supervisorId)
      // Optionally show a toast or update UI
    } finally {
      setInvitingIds((ids) => ids.filter((id) => id !== supervisorId))
    }
  }
        ...
        <DialogContent className="max-w-4xl bg-[#161A35] border-[#2A2F52] text-white">
          ...
          <Tabs defaultValue="supervisors" className="w-full">
            <TabsList className="grid w-full grid-cols-3 bg-slate-800/50 border border-slate-700">
              <TabsTrigger value="supervisors" className="data-[state=active]:bg-purple-600/20 data-[state=active]:text-purple-300">
                Supervisor Requests
              ...
              <TabsTrigger value="students" className="data-[state=active]:bg-purple-600/20 data-[state=active]:text-purple-300">
                Student Requests
              ...
              <TabsTrigger value="invite" className="data-[state=active]:bg-purple-600/20 data-[state=active]:text-purple-300">
                Send Supervisor Invites
              </TabsTrigger>
            </TabsList>
            ...
            <TabsContent value="invite" className="mt-6">
              <div className="space-y-4">
                {availableSupervisors.length === 0 ? (
                    <div className="text-center py-8 text-slate-400">
                      <GraduationCap className="h-12 w-12 mx-auto mb-4 opacity-50" />
                      <p>No supervisors available to invite.</p>
                    </div>
                ) : (
                    availableSupervisors.map((supervisor) => (
                        ...
                        >
                          <div>
                            <div className="font-semibold text-white">{supervisor.user.email}</div>
                            <div className="text-sm text-slate-400">{supervisor.rank} • {supervisor.role}</div>
                          </div>
                          <Button
                              size="sm"
                              onClick={() => handleSendInvite(supervisor.id)}
                              disabled={invitingIds.includes(supervisor.id)}
                              className="bg-purple-600/20 hover:bg-purple-600/30 text-purple-300 border-purple-500/50"
                          >
                            {invitingIds.includes(supervisor.id) ? "Inviting..." : "Invite"}
                          </Button>
                        </div>
                    ))
                )}

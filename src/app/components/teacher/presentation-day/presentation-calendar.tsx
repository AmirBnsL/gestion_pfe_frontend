...
export function PresentationCalendar({ presentationDays }: { presentationDays: PresentationDay[] }) {
  const router = useRouter()
  const [currentDate, setCurrentDate] = useState(new Date())
  const today = new Date()

  // Build presentationData from backend data
  const presentationData = Object.fromEntries(
      presentationDays.map(day => [
        day.date,
        { count: day.slots?.length ?? 0, hasPresentation: (day.slots?.length ?? 0) > 0 }
      ])
  )
